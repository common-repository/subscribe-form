...
function sfbip_ajax_process() {
    ...
    $where_to_send_subscriber = get_post_meta($post_type_id, '_sfbip_subscription_selection_dd', true);

    if( $where_to_send_subscriber == 'activecampaign'){

        define("ACTIVECAMPAIGN_URL", get_option('sfbip_ac_url'));
        define("ACTIVECAMPAIGN_API_KEY", get_option('sfbip_ac_api_key'));
        require_once("ActiveCampaignAPI/ActiveCampaign.class.php");
        $ac = new ActiveCampaign(ACTIVECAMPAIGN_URL, ACTIVECAMPAIGN_API_KEY);
        $list_id = get_post_meta($post_type_id, '_sfbip_activecampaign_lists', true);
    ...
    if( $where_to_send_subscriber == 'getresponse'){
        ...
        $api_key = esc_attr( get_option('sfbip_gr_api_key') );
        $api_url = 'http://api2.getresponse.com';
        $client = new SFBAjsonRPCClient($api_url);

        $campaign_id_is = get_post_meta($post_type_id, '_sfbip_getresponse_lists', true);
        try {
    ...
    if( $where_to_send_subscriber == 'mailchimp'){
        ...
        $sfbip_api_key = esc_attr(get_option('sfbip_mc_api_key'));
        $api = new SFBA_MCAPI($sfbip_api_key);
        $list_id = get_post_meta($post_type_id, '_sfbip_mailchimp_lists', true);
    ...
    if( $where_to_send_subscriber == 'mail'){
        ...
        $to = get_post_meta($post_type_id, '_sfbip_sent_to_email', true);
    ...
    if( $where_to_send_subscriber == 'database'){
        ...
        $resultss='';
        global $wpdb;
        $table_name = $wpdb->prefix . 'sfbip_subscribers_lists';
...
function sfbip_delete_db_record(){
    $id = sanitize_text_field($_POST['id']);
    global $wpdb;
    $table  = $wpdb->prefix . 'sfbip_subscribers_lists';
    $delete = $wpdb->query("DELETE FROM $table WHERE id=".$id);
    wp_die();
}

function sfbip_delete_db_data(){
    global $wpdb;
    $table  = $wpdb->prefix . 'sfbip_subscribers_lists';
...
add_action('wp_ajax_sfbip_ajax', 'sfbip_ajax_process');
add_action('wp_ajax_nopriv_sfbip_ajax', 'sfbip_ajax_process');

add_action('wp_ajax_sfbip_delete_db_record', 'sfbip_delete_db_record');
add_action('wp_ajax_nopriv_sfbip_delete_db_record', 'sfbip_delete_db_record');


add_action('wp_ajax_sfbip_delete_db_data', 'sfbip_delete_db_data');
add_action('wp_ajax_nopriv_sfbip_delete_db_data', 'sfbip_delete_db_data');
